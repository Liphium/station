...
// Action: wp_end
func end(c *pipeshandler.Context, warpId string) pipes.Event {

	// Get the Warp related to the packet
	warp, err := caching.GetWarp(c.Client.Session, warpId)
	if err != nil {
	...
	if err := caching.StopWarp(c.Client.Session, warpId); err != nil {
		return pipeshandler.ErrorResponse(c, localization.ErrorInvalidRequestContent, err)
	}
